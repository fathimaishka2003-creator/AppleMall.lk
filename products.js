// products.js – Products page JavaScript for AppleMall.lk
...
function filterProducts() {
  ...
  let filtered = PRODUCTS.filter(p => {
    const matchSearch = !search || p.name.toLowerCase().includes(search) || p.specs.chip.toLowerCase().includes(search);
    const matchSeries = !series || p.series === series;
    const matchStorage = !storage || p.storage === storage;
    const matchPrice = !price || p.price <= parseInt(price);
    return matchSearch && matchSeries && matchStorage && matchPrice;
  });
